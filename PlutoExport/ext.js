...
function onLoaded() {
    ...
    csInterface.evalScript('$._PPP_.getRenderPresets()', myRenderPresetsFunction);  	
...
function myRenderPresetsFunction (inputdata) {
	...
	var presets = [];
	 ...
	 for (i = 0; i < data.length; i++) {
		...
		presets.push('<button class="controlBg textStyle" id="pn'+i+'" onClick="evalScript(\'$._PPP_.plutoexport()">Export to Pluto using '+input+'</button>');
	}

// <button class="controlBg textStyle" id="btn_PPRO6"		onClick="evalScript('$._PPP_.plutoexport()')">Export to Pluto</button>
     var boilerPlate        = "Render Presets: ";
     var preset_files       = document.getElementById("preset_files");
     preset_files.innerHTML  = boilerPlate + presets;
}
